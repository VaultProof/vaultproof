      ...
      function renderPanels(orgPayload, readiness, overview) {
        ...
        if (PAGE_MODE === 'runbooks') {
          byId('runbooksSafeList').innerHTML = [
            ...
            row('Finish gate', 'npm run gate:enterprise-finish runs local smoke, APIM policy smoke, handoff gate, live app QA, and hardening status into one ok/attention/blocked release view with structured pending actions.', 'read-only', 'good'),
