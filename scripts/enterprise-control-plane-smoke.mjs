...
async function assertEnterpriseLoginRoute() {
  ...
  const supportPages = [
    ...
    },
    {
      ...
      required: ['Hardening status', 'Production verifier', 'Evidence bundle', 'Origin TLS certificate plan', 'Origin TLS preparation plan', 'Origin TLS preflight', 'TLS origin cutover', 'npm run cutover:enterprise-apim', 'Container Apps cleanup'],
    },
  ];
