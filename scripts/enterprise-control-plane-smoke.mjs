...
async function assertEnterpriseLoginRoute() {
  ...
  function assertDashboardShellTheme(path, pageHtml) {
    ...
    for (const removedShellElement of [
      ...
      'data-enterprise-sidebar="universal" data-ui-kit="shadcn-studio"',
      'nav-group-count',
      'nav-link-blurb',
  ...
  for (const dashboardPath of ['/app', '/app/', '/app/dashboard']) {
    ...
    if (dashboardHtml.indexOf('Protected API operations') > dashboardHtml.indexOf('Needs attention')) {
      throw new Error('Expected enterprise dashboard analytics card to render before attention items');
    }
