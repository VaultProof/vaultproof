...
async function assertEnterpriseLoginRoute() {
  ...
  const supportPages = [
    ...
    },
    {
      ...
      required: ['Hardening status', 'Production verifier', 'Evidence bundle', 'Handoff package', 'npm run package:enterprise-handoff', 'Handoff gate', 'npm run gate:enterprise-handoff', 'Finish gate', 'structured pending actions', 'npm run gate:enterprise-finish', 'mTLS caller-lock preparation', 'npm run prepare:enterprise-mtls', 'APIM policy template smoke', 'caller-lock header delete/override', 'npm run test:enterprise-apim-policies', 'Origin TLS certificate plan', 'Origin TLS preparation plan', 'Origin TLS preflight', 'TLS origin cutover', 'npm run cutover:enterprise-apim', 'Container Apps cleanup'],
    },
  ];
