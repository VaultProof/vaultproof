...
- \`https://${adminDomain}/\` uses the same light enterprise app-shell theme tokens as the customer dashboard and includes a staff Control Center with signed-on business/user KPIs, SSO and pending-invite posture, total API calls, API calls by business, account-mix charts, and traffic-health charts backed by \`project_access_log_daily_rollups\`.
