...
- \`https://${edgeDomain}/app/docs\` is the enterprise-only documentation index for purchased customers: setup, technical guide, security review, runbooks, evidence, provider slots, scanner, SSO notes, key exposure response, proof boundary, and links to the customer-safe operating pages without mixing in public developer docs.
...
- \`https://${edgeDomain}/app/testers\` is the paid-pilot tester readiness board: browser-local tester roster, login/scenario status, scenario assignment, guided session plan, blocker notes, customer-safe feedback, copyable session brief, and copyable \`vaultproof_enterprise_paid_pilot_tester_readiness\` JSON without secrets.
...
- \`https://${edgeDomain}/app/security-review\` is the buyer security packet: concise architecture summary, control coverage, evidence links, searchable/filterable open review items, common customer answers, known limitations, secret exclusions, copyable full security/procurement review text, and a copyable focused review brief for the currently filtered blockers/limitations.
