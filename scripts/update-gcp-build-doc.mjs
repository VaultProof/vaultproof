...
- Customer-facing enterprise pages live under \`https://${edgeDomain}\`; staff-only operator pages live under \`https://${adminDomain}\`.
- \`https://${adminDomain}/app/launch\` is the staff-only go/no-go launch board. \`https://${edgeDomain}/app/launch\` is intentionally removed from the customer enterprise host and should return 404.
- \`https://${adminDomain}/app/demo\` is the staff-only buyer walkthrough: live workspace facts, proof path, identity/OAuth proof kit, key-rotation proof kit, pilot operations proof kit, API proxy self-test kit, monitoring evidence kit, paid-pilot tester readiness, paid-customer onboarding, safety guardrails, objection answers, paid-pilot close steps, and a copyable demo talk track generated without secrets. \`https://${edgeDomain}/app/demo\` is intentionally removed from the customer enterprise host and should return 404.
...
- \`https://${edgeDomain}/app/inventory\` is the customer API inventory board: metadata-only API surfaces from existing projects, provider slots, manual API key records, caller-lock policy, project health, and access-log rollups; browser-local owner/environment/risk/review annotations; protected/manual-key/missing-provider/needs-sealed-ingest/policy-incomplete/no-traffic/stale/review-due posture; workflow links; and copyable \`vaultproof_enterprise_api_inventory\` JSON without secrets.
...
- \`https://${edgeDomain}/app/release\` is the release evidence center: browser-local build/image tag, change summary, approver, verifier, verification status, rollout state, rollback owner/path, and customer-safe notes; it exports \`vaultproof_enterprise_release_evidence\` JSON without secrets and feeds Evidence, Demo, and Security Review.
- \`https://${edgeDomain}/app/testers\` is the paid-pilot tester readiness board: browser-local tester roster, login/scenario status, scenario assignment, blocker notes, customer-safe feedback, and copyable \`vaultproof_enterprise_paid_pilot_tester_readiness\` JSON without secrets.
- \`https://${adminDomain}/app/onboarding\` is the staff-only paid-customer onboarding board: browser-local activation owners, enterprise admin login handoff, first workload owner, support handoff, capacity/renewal review, key posture, customer testing window, and copyable \`vaultproof_enterprise_paid_onboarding\` JSON without secrets. \`https://${edgeDomain}/app/onboarding\` is intentionally removed from the customer enterprise host and should return 404.
...
- \`https://${edgeDomain}/app/plans\` is the buyer package view: rollout posture, paid-pilot commercial package, contract guardrails, security boundaries, and direct links into evidence, support, technical guide, and runbooks.
...
- \`https://${adminDomain}/app/pilot-success\` is the staff-only pilot success tracker: live checks, browser-local customer milestones, evidence links, blockers, expansion/no-go readiness, and copyable weekly customer update without secrets. \`https://${edgeDomain}/app/pilot-success\` is intentionally removed from the customer enterprise host and should return 404.
- \`https://${adminDomain}/app/support\` is the staff-only launch support room: founder-led support scope, optional 24-hour incident-response boundary, staff/admin boundary, read-only default, approval-gated actions, support handoff checklist, and copyable support brief without secrets. \`https://${edgeDomain}/app/support\` is intentionally removed from the customer enterprise host and should return 404.
...
The demo slice starts from existing data instead of new infrastructure: it derives inventory rows from projects, provider slots, project policies, project health, access logs, and rollups; adds browser-local/manual annotations for owner, environment, business service, sensitivity, risk, review status, next review date, and notes; lets operators add manual API key records with provider, label, fingerprint/last four, location, upstream scope, rotation status, and review status without storing raw keys; flags manual keys that still need sealed provider-slot ingest, missing provider slots, stale APIs, no recent traffic, policy gaps, blocked rows, exceptions, and review-due items; then includes the summary in the evidence packet under \`api_inventory\`. Persistent audited inventory tables, sealed-key intake from the inventory page, CSV/OpenAPI import, and automatic discovery can follow after the customer demo slice.
...
\`https://${edgeDomain}/app/testers\` gives operators a customer-safe board for tomorrow's paid-user testers. It records tester name/email, team, role, scenario, login/scenario status, VaultProof owner, blocker note, and customer-safe feedback.
