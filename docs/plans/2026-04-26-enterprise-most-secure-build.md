...
- `npm run gate:enterprise-finish` runs the finish-line release view across local enterprise smoke, APIM policy smoke, handoff gate, live app QA, and read-only hardening status, returning `ok`, `attention`, or `blocked` with a JSON result artifact, named pending actions, and exact `BLOCKER`/`WARN` issue lines.
...
- `npm run prepare:enterprise-internal-admin` checks employee allowlist env, internal admin schema tables, customer-host separation, and admin-host auth behavior before exposing `admin.vaultproof.dev`.
...
- The enterprise control plane also has the first VaultProof employee-only internal admin console slice ready for `admin.vaultproof.dev`: business/org/user/project/SSO/support/audit visibility, persistent service-role-only employee access audit logging, explicit employee email/domain allowlist auth, no browser service-role exposure, no customer-dashboard links, and safe support/invitation/status actions behind a disabled-by-default action gate plus approval secret. Live DNS/Front Door exposure is verified; production employee sign-in still needs an allowlisted-account check.
...
- Enterprise Provider Slots, Evidence, Security Review, Support, and Runbooks now include a customer-safe key exposure response workflow: linked scanner findings, provider-slot containment status, emergency revoke path, rotation scope, proof-boundary language, incident JSON, and audit/activity export links without exposing raw provider material.
...
- [x] Configure APIM policies for JWT validation, coarse rate limits, quotas, request size limits, and observability. Deployable APIM policy support now includes JWT validation, coarse limits, quota, request-size guard, provider-secret header stripping, spoofable caller-lock header stripping before trusted gateway/device/mTLS header overrides, APIM marker, APIM origin-lock forwarding, API operations, and App Insights diagnostics. `npm run prepare:enterprise-apim-jwt` now plans the final Supabase-session or direct-Entra issuer/audience settings before live APIM JWT validation is enabled.
...
44. [ ] Live browser QA after each major deploy: automated live app QA passed after Key Vault SKR migration, Managed HSM soft-delete, Container Apps scale-to-zero, boot diagnostics enablement, SSH bootstrap closure, internal-admin launch hardening, destructive-action evidence gates, and login recovery/magic-link fixes. Latest sweep on 2026-05-03 checked 19 paths and 22 links with `production_ready: true` and `security_profile: azure-confidential-production`; authenticated demo-user clickthrough remains skipped until credentials are supplied.
...
- Default release posture is read-only. Employee console views are audit logged. Safe writes require the disabled-by-default action gate plus approval secret; destructive workflows require second-employee approval and remain dry-run only.
...
4. [ ] Configure `admin.vaultproof.dev` in Front Door/DNS and employee allowlist env vars. In progress: live Front Door/DNS exposure now reaches the control-plane service; unauthenticated `/` redirects to `/app/login?internal_admin=true`, and unauthenticated `/api/v1/internal-admin/overview` returns the employee-auth `401`. `render-control-plane-env.sh` renders the internal admin hostname, preview flag, employee allowlists, disabled-by-default write-action gate, and approval secret into the VM control-plane env file; `npm run prepare:enterprise-internal-admin` now asserts customer-host separation, the exact employee-login redirect, and API auth behavior. Remaining operator check: sign in with an account covered by the production employee allowlist and verify the overview loads.
