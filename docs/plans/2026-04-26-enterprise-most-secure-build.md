...
4. [ ] Configure `admin.vaultproof.dev` in Front Door/DNS and employee allowlist env vars. In progress: `render-control-plane-env.sh` now renders the internal admin hostname, preview flag, employee allowlists, disabled-by-default write-action gate, and approval secret into the VM control-plane env file; `npm run prepare:enterprise-internal-admin` performs a read-only launch preflight for env/schema/host-separation/auth checks; live Front Door/DNS exposure remains pending.
