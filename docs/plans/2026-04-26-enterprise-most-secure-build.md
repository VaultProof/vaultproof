...
- `npm run gate:enterprise-finish` runs the finish-line release view across local enterprise smoke, APIM policy smoke, handoff gate, live app QA, and read-only hardening status, returning `ok`, `attention`, or `blocked` with a JSON result artifact and named pending actions.
...
31. [ ] Live browser QA after each major deploy: run the automated live app QA, then manually login as demo user and click through sidebar/subnav links when visual regressions or browser-only session behavior are in scope.
