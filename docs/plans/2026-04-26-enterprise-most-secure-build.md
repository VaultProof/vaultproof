...
- `npm run test:enterprise-apim-policies` validates the VaultProof-managed, customer-managed, device, and mTLS APIM policy templates before handoff.
...
26. [ ] Live browser QA after each major deploy: run the automated live app QA, then manually login as demo user and click through sidebar/subnav links when visual regressions or browser-only session behavior are in scope.
