...
- [ ] Add mTLS after private networking is stable. In progress: `npm run prepare:enterprise-mtls` now computes the certificate caller-lock policy snippet and APIM/customer-gateway header contract without changing live traffic; actual APIM/customer gateway client-certificate enforcement remains pending private networking and customer certificate rollout.
...
25. [ ] Live browser QA after each major deploy: run the automated live app QA, then manually login as demo user and click through sidebar/subnav links when visual regressions or browser-only session behavior are in scope.
