...
| Staff admin console | `https://admin.vaultproof.dev/` | Employee-only console for creating enterprise businesses, managing users/invites, setting SSO metadata, and copying per-business login links. It requires a Supabase user session, a `vaultproof.dev` email domain, and explicit employee email/domain allowlist. |
