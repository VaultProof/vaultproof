...
| Provider Slots | `/app/keys` | Active providers, provider preset catalog, non-secret extra-header templates, emergency revoke, rotation checklist, and Secure Key Release notes. |
...
| Staff admin console | `https://admin.vaultproof.dev/` | Employee-only console for creating enterprise businesses, managing users/invites, setting SSO metadata, copying per-business login links, and viewing the chart-forward Control Center for signed-on businesses, users, daily API-call trend, API calls by business, account mix, traffic health, launch blockers, SSO posture, and pending invites. It requires a Supabase user session, a `vaultproof.dev` email domain, and explicit employee email/domain allowlist. |
...
- `VAULTPROOF_INTERNAL_ADMIN_EMAILS=employee@vaultproof.dev,...` or `VAULTPROOF_INTERNAL_ADMIN_DOMAINS=vaultproof.dev`; non-`vaultproof.dev` emails/domains are rejected even if accidentally configured.
