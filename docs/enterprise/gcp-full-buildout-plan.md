...
- Paid-user entitlements are browser-local for the demo; durable contract source-of-truth, billing API integration, invoice automation, hard limit enforcement, and audited amendment history remain follow-up work.
...
- Paid-user entitlements at `https://enterprise.vaultproof.dev/app/entitlements` are reviewed before onboarding for contract status, capacity allowance, billing owner, success owner, support tier, renewal date, incident-response boundary, and commercial handoff metadata.
