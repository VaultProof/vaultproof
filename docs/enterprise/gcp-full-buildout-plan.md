...
- Customer security review packet at `https://enterprise.vaultproof.dev/app/security-review` for architecture, controls, evidence links, searchable/filterable open items, focused review brief copy, common buyer answers, known limitations, and secret exclusions
...
- Paid-user entitlements page at `https://enterprise.vaultproof.dev/app/entitlements` for contract status, capacity envelope, usage meters, expansion recommendation, billing owner, success owner, support tier, renewal/review date, incident-response boundary, commercial handoff metadata, amendment/renewal log, copyable capacity brief, and customer-safe JSON proof
- Staff paid-customer onboarding board at `https://admin.vaultproof.dev/app/onboarding` for activation owners, enterprise login handoff, first workload scope, support handoff, capacity/renewal review, key posture, customer testing window, role-specific security/platform/app/billing/support tasks, copyable task brief, and customer-safe JSON proof
- Staff-only paid-pilot proposal builder at `https://admin.vaultproof.dev/app/pilot` for first workload scope, expected volume, monthly price, sales commission math, support boundary, incident-response terms, success metric, and close steps
- Staff pilot success tracker at `https://admin.vaultproof.dev/app/pilot-success` for live checks, browser-local milestones, evidence links, blockers, weekly customer update copy, structured expansion/hold/no-go decision, and copyable decision brief
- Paid-pilot tester readiness board at `https://enterprise.vaultproof.dev/app/testers` for browser-local tester roster, login/scenario status, scenario assignment, guided session plan, customer-safe feedback, blockers, copyable session brief, and JSON evidence
...
- Paid-user entitlements are browser-local for the demo; durable contract source-of-truth, billing API integration, invoice automation, hard limit enforcement, and audited amendment records remain follow-up work.
...
Customer packaging status: customer-facing buyer pages are on `https://enterprise.vaultproof.dev`, while staff-only demo, onboarding, support, pilot proposal, pilot-success, and launch controls are on `https://admin.vaultproof.dev`. `https://enterprise.vaultproof.dev/app/plans` now carries the first sellable paid-pilot package view, `https://enterprise.vaultproof.dev/app/entitlements` records paid-user contract status, capacity, usage meters, expansion recommendation, hard-limit boundary, owners, support tier, renewal date, incident-response boundary, commercial handoff metadata, and amendment/renewal log, `https://admin.vaultproof.dev/app/onboarding` turns accepted entitlements into activation owners, login handoff, first workload scope, support handoff, key posture, testing-window evidence, and role-specific customer task ownership, `https://enterprise.vaultproof.dev/app/security-review` gives buyers a copyable security/procurement review packet with filtered open-item triage and focused brief copy, `https://enterprise.vaultproof.dev/app/release` records build/approval/verification/rollback proof after each deploy, `https://enterprise.vaultproof.dev/app/testers` prepares the paid-pilot tester roster, guided-session plan, and session brief, `https://admin.vaultproof.dev/app/pilot` creates the first-workload proposal with price, commission, support, incident-response, and close-step terms, and `https://admin.vaultproof.dev/app/pilot-success` tracks weekly proof, milestones, blockers, expansion/hold/no-go decision metadata, and decision brief copy after kickoff. It keeps automated billing and hard plan enforcement out of scope for Goal 1; capacity, support cadence, retention, SSO depth, and dedicated-runtime terms stay contract-controlled until billing APIs exist.
...
- Track package, contract status, monthly call allowance, provider-slot allowance, seat allowance, support tier, incident-response add-on status, runtime type, renewal/review date, billing owner, success owner, retention label, commercial handoff, amendment history, renewal watch, and customer-safe notes.
...
- Show paid users whether usage is inside contract: call/provider-slot/seat meters, capacity status, remaining calls, expansion recommendation, usage actions, and the manual hard-limit boundary.
- Export `vaultproof_enterprise_entitlements` JSON without card numbers, bank data, raw provider keys, encrypted shares, Supabase service-role keys, browser sessions, OAuth secrets, origin-lock values, signing secrets, runtime-token secrets, or unwrap roots.
...
- Built: compute capacity status, remaining calls, provider-slot and seat utilization, expansion recommendation, usage actions, and hard-limit boundary; render usage meters; and provide a copyable capacity brief.
- Built: record commercial handoff metadata for invoice status, PO status, procurement owner, payment terms, expansion review date, billing-safe note, and customer-safe next actions without exposing card/bank data or secrets.
- Built: add browser-local amendment and renewal log with change type, status, owner, effective date, renewal/expansion due status, amendment blockers, and copyable customer-safe summary.
...
- Track role-specific activation tasks for security, platform, app owner, billing, and support contacts, with owner, due date, status, note, and copyable task brief.
...
- Built: add browser-local role task checklist for security, platform, app owner, billing, and support contacts; include `role_task_summary` and `role_task_checklist` in packet version 2; and provide a copyable customer-safe task brief.
...
- Add persistent audited onboarding records tied to staff-admin business accounts, customer invites, durable task assignment, customer notifications, and acceptance history.
...
- Add persistent contract entitlements with billing integration, invoice automation, hard limit enforcement, amendment history, renewal reminders, and staff-admin management.
...
- Paid-pilot tester readiness at `https://enterprise.vaultproof.dev/app/testers` is reviewed before guided customer testing for roster, login pass, scenario assignment, guided session window, facilitator, customer owner, success criteria, feedback notes, and blocker ownership.
- Paid-user entitlements at `https://enterprise.vaultproof.dev/app/entitlements` are reviewed before onboarding for contract status, capacity allowance, billing owner, success owner, support tier, renewal date, incident-response boundary, commercial handoff metadata, and amendment/renewal log.
