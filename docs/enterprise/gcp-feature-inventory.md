...
| Customer evidence packet | Built | `https://enterprise.vaultproof.dev/app/evidence` assembles readiness, go/no-go launch decision and blockers, access-review, audit export links, API inventory proof, policy drift proof, provider posture, rollout workflow, and a copy/download JSON proof packet scoped to the selected organization without exposing secrets. |
...
| Integration rollout manager | Not built | Planned enterprise feature for moving one customer workload from direct provider calls into VaultProof with environment-by-environment rollout state, integration mode, app/gateway owners, target date, canary percent, test status, rollback path, blockers, snippets, and customer-safe evidence. First demo slice should use existing projects, provider slots, API inventory rows, policy drift, launch go/no-go, activity/audit evidence, and browser-local/manual rollout notes before a persistent audited rollout table. No raw keys, bearer tokens, OAuth secrets, SAML material, request bodies, responses, or payloads belong in rollout records. |
...
| Projects page bootstrap load | Built | `https://enterprise.vaultproof.dev/app/projects`, `https://enterprise.vaultproof.dev/app/inventory`, `https://enterprise.vaultproof.dev/app/policy`, `https://enterprise.vaultproof.dev/app/keys`, and `https://enterprise.vaultproof.dev/app/activity` now load orgs, projects, provider slots, and overview stats from `GET /api/v1/enterprise/projects/bootstrap`. Deployed consolidated bootstrap RPC median is about 383 ms for authenticated bootstrap; direct stats overview remains about 474 ms median. |
