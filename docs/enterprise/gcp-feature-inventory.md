...
| Customer evidence packet | Built | `https://enterprise.vaultproof.dev/app/evidence` assembles readiness, go/no-go launch decision and blockers, access-review, audit export links, API inventory proof, provider posture, rollout workflow, and a copy/download JSON proof packet scoped to the selected organization without exposing secrets. |
...
| Policy drift and exceptions | Not built | Planned enterprise feature for showing missing or weak controls across API inventory, project policy, provider-slot material mode, login/readiness evidence, Cloud Armor checks, rotation, and review dates. First demo slice should derive drift rows from existing data and browser-local inventory annotations, then let operators record metadata-only exceptions with owner, reason, risk, compensating control, expiration date, and next action. Persistent audited exceptions, approval workflow, expiry reminders, and policy-as-code export can follow. |
...
| Projects page bootstrap load | Built | `https://enterprise.vaultproof.dev/app/projects`, `https://enterprise.vaultproof.dev/app/inventory`, `https://enterprise.vaultproof.dev/app/keys`, and `https://enterprise.vaultproof.dev/app/activity` now load orgs, projects, provider slots, and overview stats from `GET /api/v1/enterprise/projects/bootstrap`. Deployed consolidated bootstrap RPC median is about 383 ms for authenticated bootstrap; direct stats overview remains about 474 ms median. |
