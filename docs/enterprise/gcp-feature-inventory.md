...
| Customer evidence packet | Built | `https://enterprise.vaultproof.dev/app/evidence` assembles readiness, go/no-go launch decision and blockers, access-review, audit export links, provider posture, rollout workflow, and a copy/download JSON proof packet scoped to the selected organization without exposing secrets. |
...
| API inventory management | Not built | Planned enterprise feature for cataloging API surfaces by project, provider slot, owner, environment, risk, data sensitivity, caller-lock policy, last-seen traffic, review status, and evidence/export path. First demo slice should derive rows from existing projects, provider slots, policy, and access-log rollups, then add browser-local/manual annotations before a persistent audited inventory table. No raw keys, bearer tokens, OAuth client secrets, request bodies, or payloads belong in inventory records. |
...
- Build the first API inventory management slice for the demo: derived inventory rows, owner/risk/review annotations, traffic posture, stale/orphaned flags, and evidence export without secrets.
