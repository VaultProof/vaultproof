...
| Azure Front Door | External HTTPS Load Balancer + Cloud Armor | Map `enterprise.vaultproof.dev` after DNS cutover for customers. Map `admin.vaultproof.dev` only for the employee admin console with the admin certificate and allowlist enabled. |
